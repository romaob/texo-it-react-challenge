...
describe('PageWrapper tests', () => {
  it('should render the component correctly crashing', () => {
    render(
        <PageWrapper title={TEST_TITLE}>
            {CHILD}
        </PageWrapper>
    );
    expect(screen.getByTestId('page-wrapper')).toBeInTheDocument();
    expect(screen.getByTestId('page-wrapper-app-name')).toBeInTheDocument();
    expect(screen.getByTestId('page-wrapper-app-name')).toHaveTextContent(strings.appName);
    ...
    expect(screen.getByTestId('page-wrapper-title')).toHaveTextContent(TEST_TITLE);
    ...
    it('should toggle the data-show attribute on the toggle menu button', () => {
        ...
        expect(screen.getByTestId('page-wrapper-header-button')).toHaveAttribute('data-show', 'false');
        act(() => {
            screen.getByTestId('toggle-menu').click();
        });
        expect(screen.getByTestId('page-wrapper-header-button')).toHaveAttribute('data-show', 'true');
        act(() => {
            screen.getByTestId('toggle-menu').click();
        });
        expect(screen.getByTestId('page-wrapper-header-button')).toHaveAttribute('data-show', 'false');
